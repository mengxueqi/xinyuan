...
class WebCollector(BaseCollector):
    ...
    def _extract_list_items(
        ...
        for anchor in soup.find_all("a", href=True):
            ...
            dedupe_key = self._normalize_url_for_dedupe(absolute_url)
    ...
    def _is_candidate_link(
        ...
        if absolute_parts.netloc and not same_host:
            return False
        if self._is_same_document_url(source_url, absolute_url):
            return False
    ...
    def _normalize_url_for_dedupe(self, value: str) -> str:
        parsed = urlparse(value)
        return parsed._replace(fragment="").geturl().rstrip("/")

    def _is_same_document_url(self, source_url: str, absolute_url: str) -> bool:
        source_parts = urlparse(source_url)
        target_parts = urlparse(absolute_url)
        return (
            source_parts.netloc == target_parts.netloc
            and source_parts.path.rstrip("/") == target_parts.path.rstrip("/")
            and source_parts.query == target_parts.query
            and not target_parts.fragment
        )
    ...
    def _fetch_article_metadata(
        ...
        lead_title = self._extract_lead_title(content_text)
        if self._should_prefer_lead_title(article_url, title, lead_title):
            title = lead_title
    ...
    def _extract_lead_title(self, content_text: str | None) -> str | None:
        if not content_text:
            return None
        ...
        candidate = self._clean_title(text[: min(marker_positions)])
        if candidate and 4 <= len(candidate) <= 120:
            return candidate
        return None

    def _should_prefer_lead_title(
        self,
        article_url: str,
        ...
        if "newsxq.aspx" in lowered_url:
            return True
        generic_title_hints = (
            "\u4e0a\u6d77\u51ef\u8d5b\u751f\u7269\u6280\u672f\u80a1\u4efd\u6709\u9650\u516c\u53f8",
            "\u51ef\u8d5b\u751f\u7269cathay biotech inc.",
        )
        return current_title.strip().lower() in generic_title_hints
