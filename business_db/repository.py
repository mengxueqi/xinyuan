...
class BusinessDatabase:
    ...
    def fetch_recent_changes(
        ...
        if company_name and company_name != "All":
            ...
            params.append(company_name)
        if start_date:
            query += " AND substr(COALESCE(NULLIF(detected_at, ''), batch_date), 1, 10) >= ?"
            params.append(start_date.isoformat())
        if end_date:
            query += " AND substr(COALESCE(NULLIF(detected_at, ''), batch_date), 1, 10) <= ?"
            params.append(end_date.isoformat())
        query += " ORDER BY COALESCE(NULLIF(detected_at, ''), batch_date) DESC, importance_score DESC LIMIT ?"
        params.append(limit)
    ...
    def fetch_recent_insights(
        ...
        if end_date:
            query += " AND substr(COALESCE(NULLIF(detected_at, ''), batch_date), 1, 10) <= ?"
            params.append(end_date.isoformat())
        query += " ORDER BY importance_score DESC, COALESCE(NULLIF(detected_at, ''), batch_date) DESC LIMIT ?"
        params.append(limit)
            ...
            rows = connection.execute(
                ...
                WHERE company_name IN (SELECT company_name FROM companies)
                  ...
                  AND NOT EXISTS (
                      SELECT 1
                      FROM sources
                      WHERE sources.url = processed_events.url
                        AND lower(sources.parser_type) IN (
                            'news_page',
                            'news_hub_page',
                            'ir_page',
                            'notice_hub_page'
