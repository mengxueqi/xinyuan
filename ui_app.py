...
def get_latest_report_covered_date() -> date | None:
    ...
    try:
        ...
        return parse_report_date(latest_report.name)
    return parse_covered_date(report_content) or parse_report_date(latest_report.name)
...
def build_dashboard_change_rows(records: list[dict]) -> list[dict]:
    rows = []
    for record in records:
        event_types = get_change_event_types(record)
        rows.append(
            {
                **record,
                "company_name": get_change_company_display(record),
                "detected_at": record.get("detected_at") or record.get("batch_date"),
                "published_date": get_change_published_date(record),
...
def render_new_events(records: list[dict]) -> None:
    if not records:
        st.info("No new events for this filter.")
        return

    st.caption(f"Showing {len(records)} new-event rows. Each event is displayed separately.")
    render_dataframe_with_links(
        build_dashboard_change_rows(records),
        [
            "detected_at",
            "published_date",
            "company_name",
...
def render_change_analysis(records: list[dict]) -> None:
    if not records:
        st.info("No change analysis for this filter.")
        return

    st.caption(f"Showing {len(records)} analysis rows. Each event is displayed separately.")
    render_dataframe_with_links(
        build_dashboard_change_rows(records),
        [
            "detected_at",
...
def get_dashboard_filters(database: BusinessDatabase) -> tuple[str, date, date, int]:
    companies = ["All"] + database.list_companies()
    columns = st.columns([1, 1])
    selected_company = columns[0].selectbox("Company", companies, index=0)
    window_end = get_latest_report_covered_date() or date.today()
    window_start = window_end - timedelta(days=DASHBOARD_NEW_WINDOW_DAYS - 1)
    row_limit = columns[1].slider("Rows", min_value=10, max_value=200, value=100, step=10)
    return selected_company, window_start, window_end, row_limit
...
def render_dashboard(database: BusinessDatabase) -> None:
    selected_company, window_start, window_end, row_limit = get_dashboard_filters(database)
    changes = database.fetch_recent_changes(
        selected_company,
        start_date=window_start,
        end_date=window_end,
        limit=row_limit,
    )
    analysis = database.fetch_recent_insights(
        selected_company,
        start_date=window_start,
        end_date=window_end,
        limit=row_limit,
    ...
    with tabs[0]:
        st.caption(
            f"Latest report covered date: {window_end.isoformat()}. "
            f"Weekly New window: {window_start.isoformat()} to {window_end.isoformat()}. "
